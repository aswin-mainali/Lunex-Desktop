...
export const api = {
  ...
  async routeCommand(command: string, confirmed = false, source?: 'text_command' | 'voice_command') { return request<CommandResponse>('/commands/route', { method: 'POST', body: JSON.stringify({ command, confirmed, source }) }); },
  ...
  async simulateWake() { return request<ActivationStatus>('/activation/wake/simulate', { method: 'POST' }); },
  async simulateClap() { return request<ActivationStatus>('/activation/clap/simulate', { method: 'POST' }); },
  ...
  async tasks(status?: string) { return request<Task[]>(`/tasks${status ? `?status=${encodeURIComponent(status)}` : ''}`); },
  async addTask(task: TaskInput) { return request<Task>('/tasks', { method: 'POST', body: JSON.stringify(task) }); },
  async updateTask(id: number, values: Partial<Task>) { return request<Task>(`/tasks/${id}`, { method: 'PATCH', body: JSON.stringify(values) }); },
  async deleteTask(id: number) { return request<{ deleted: boolean; id: number }>(`/tasks/${id}`, { method: 'DELETE' }); },
};
